defmodule MaxGallery.GroupDuplicateBugTest do
  ...
  test "group duplication bug: folder with 2+ items including subfolders", %{msg: msg} do
    ...
    assert {:ok, _file1_id} =
             Context.cypher_insert(path, test_user, "key", group: parent_id, name: "file1.txt")
    ...
    assert {:ok, _file2_id} =
             Context.cypher_insert(path, test_user, "key", group: child_id, name: "file2.txt")

    # Verify the structure before duplication
    assert {:ok, parent_contents} =
             Context.decrypt_all(test_user, "key", group: parent_id, lazy: true)

    # 1 file + 1 group
    assert length(parent_contents) == 2

    # Find the child group and file
    file1 =
      Enum.find(parent_contents, fn item -> Map.has_key?(item, :ext) && item.name == "file1" end)

    child_group =
      Enum.find(parent_contents, fn item -> !Map.has_key?(item, :ext) && item.name == "Child" end)

    assert file1 != nil
    assert child_group != nil

    # Verify child group has content
    assert {:ok, child_contents} =
             Context.decrypt_all(test_user, "key", group: child_group.id, lazy: true)
    ...
    assert Map.has_key?(file2, :ext)

    # Now duplicate the parent group - this should trigger the bug
    assert {:ok, new_parent_id} =
             Context.group_duplicate(test_user, parent_id, %{group_id: nil}, "key")

    # Verify the duplicated structure
    assert {:ok, new_parent_contents} =
             Context.decrypt_all(test_user, "key", group: new_parent_id, lazy: true)

    # Should still be 1 file + 1 group
    assert length(new_parent_contents) == 2

    # Find the duplicated items
    new_file1 =
      Enum.find(new_parent_contents, fn item ->
        Map.has_key?(item, :ext) && item.name == "file1"
      end)

    new_child_group =
      Enum.find(new_parent_contents, fn item ->
        !Map.has_key?(item, :ext) && item.name == "Child"
      end)
    ...
    assert !Map.has_key?(new_child_group, :ext),
           "Child group should not have :ext field (should not be treated as file)"

    # Verify the child group's contents
    assert {:ok, new_child_contents} =
             Context.decrypt_all(test_user, "key", group: new_child_group.id, lazy: true)
    ...
    # Should be a file
    assert Map.has_key?(new_file2, :ext)
